...
/**
 * Walk a Screen node's body/footer/children and return a short, human-
 * readable label (the first Heading.text or Eyebrow.text — falls back to
 * the screen kind). Used for chart axis labels and tooltips.
 */
...
export async function GET(
  ...
  // Build a per-variant lookup of screen metadata + segmentation field +
  // result-screen presence. We need this to translate raw screen_index
  // hits into labels, drop-off charts, and result distribution.
  ...
  for (const v of variants) {
    ...
    // Build value→label map for the segmentation field's options. Lets
    // us show a human label in the donut even when the persisted answer
    // is a stable value slug.
  ...
  // Per-variant aggregate + analytics derivatives.
  ...
  const perVariant = variants.map((v) => {
    ...
    // 30-day sparkline of started sessions per day (oldest → newest).
    ...
    // Drop-off: how many sessions reached each screen. Reach = at least
    // one response row for that screen_index (or session's last answered
    // screen index >= screen_index). For unanswered final screens (e.g.
    // intro/result with no fields), we approximate using completion.
    ...
    for (const sess of vSessions) {
      ...
      const lastIdx = sessResp.length
        ...
        : -1;
      // Mark as reached every index ≤ lastIdx.
      for (let i = 0; i <= Math.min(lastIdx, meta.screens.length - 1); i++) {
        reachByIndex[i] += 1;
      }
      // Plus any screens after the last answered one if the session
      // completed (the user passed through them but they had no fields).
    ...
    const dropOff = meta.screens.map((s, i) => ({
    ...
    // Per-source breakdown for this variant.
    ...
    // Result distribution: only meaningful if the variant has a result
    // screen AND we found a segmentation field. For each completed
    // session, look up its answer to that field and count.
  ...
  // Overall per-source (all variants combined).
  ...
  const perSourceTotal = [...totalSourceMap.values()].map((r) => ({
    ...r,
    completion_rate: r.started ? r.completed / r.started : 0,
  }));
  ...
  // Build the response/session list with answer + per-screen dwell breakdown.
  const responseList = sessions.map((sess) => {
    ...
    const answers = sessResp.map((r) => {
      const screen = meta?.screens.find(
        (s) => s.screen_index === r.screen_index,
      );
      const ans = (r.answer ?? {}) as Record<string, unknown>;
      // Prefer mapping value→label from the segmentation field options;
      // for other fields just stringify. Multi-field screens get joined.
